 ...
 */
fun <C : Any> PagesNavigator<C>.navigate(transformer: (stack: List<C>) -> List<C>) {
    navigate(transformer = { pages ->
        val newItems = transformer(pages.items)
        pages.copy(
            items = newItems,
 ...
 */
fun <C : Any> PagesNavigator<C>.replaceAll(vararg configurations: C) {
    navigate(
        transformer = { configurations.toList() }
    )
}
